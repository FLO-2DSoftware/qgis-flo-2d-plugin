...
class TestFlo2dGlobalSelfHelpKit(unittest.TestCase):
    ...
    def test_global_chan(self):                                                       # Failed (chanbank passes on its own)
        self.f2g.export_chan(EXPORT_GLOBAL_DATA_DIR)
        infile1 = os.path.join(GLOBAL_DATA_DIR, "CHAN.DAT")
        infile2 = os.path.join(GLOBAL_DATA_DIR, "CHANBANK.DAT")
        outfile1 = os.path.join(EXPORT_GLOBAL_DATA_DIR, "CHAN.DAT")
        outfile2 = os.path.join(EXPORT_GLOBAL_DATA_DIR, "CHANBANK.DAT")
        in_chan, out_chan = compare_files(infile1, outfile1)
        in_chanbank, out_chanbank = compare_files(infile2, outfile2)
        self.assertEqual(in_chan, out_chan)
        self.assertEqual(in_chanbank, out_chanbank)
