...
class Flo2D(object):
    ...
    @connection_required
    def show_fpxsec_hydrograph(self, fid=None, extra=""):
        """
        Show the floodplain cross-section hydrograph from HYCROSS.OUT
        """
        self.uc.bar_info("Reading Floodplain Cross Section data from " + extra)
        self.f2d_widget.fpxsec_editor.show_hydrograph(self.cur_profile_table, fid, extra)
        self.cur_profile_table = None
    ...
    def show_levee_elev_tool(self):
        ...
        if self.dlg_levee_elev is not None:
            if self.dlg_levee_elev.isVisible():
                self.dlg_levee_elev.showNormal()
                self.dlg_levee_elev.raise_()
                self.dlg_levee_elev.activateWindow()
                return

        self.dlg_levee_elev = LeveesToolDialog(self.con, self.iface, self.lyrs)
        self.dlg_levee_elev.show()
    ...
    def show_help():
        QDesktopServices.openUrl(
            QUrl("https://documentation.flo-2d.com/Build25/flo-2d_plugin/user_manual/toolbar/index.html"))
    ...
    def get_feature_profile(self, table, fid, extra):
        ...
        if table == 'fpxsec':
            self.cur_profile_table = table
            self.show_fpxsec_hydrograph(fid, extra)
