...
class MudAndSedimentDialog(qtBaseClass, uiDialog):
    def __init__(self, con, iface, lyrs):
        ...
        self.sed_rating_curve_tblw.setColumnWidth(1, 100)
        ...
        self.sed_rating_curve_tblw.cellChanged.connect(self.sed_rating_curve_tblw_cellchanged)
        ...
        self.sed_size_fraction_dp_tblw.setItemDelegate(FloatDelegate(5, self.sed_size_fraction_dp_tblw))
        # self.sed_size_fraction_dp_tblw.itemChanged.connect(self.sed_size_fraction_dp_tblw_changed)

        self.sed_rating_curve_dp_tblw.setItemDelegate(FloatDelegate(5, self.sed_rating_curve_dp_tblw))
        ...
        self.gutils.execute(
            """CREATE TABLE sed_group_frac_data_tmp 
        ...
        self.gutils.execute(
            "INSERT INTO sed_group_frac_data_tmp SELECT fid, dist_fid, sediam, sedpercent FROM sed_group_frac_data"
        )
        ...
        self.gutils.execute(
            """CREATE TABLE sed_group_cells_tmp 
                                ( fid INTEGER PRIMARY KEY NOT NULL,     
                                  grid_fid INTEGER,
                                  area_fid INTEGER
                                )
        ...
        self.gutils.execute(
            """CREATE TABLE sed_supply_frac_data_tmp 
                                ( fid INTEGER PRIMARY KEY NOT NULL,     
                                  dist_fid INTEGER,
                                  ssediam REAL,
                                  ssedpercent REAL
        ...
        self.gutils.execute(
            "INSERT INTO sed_supply_frac_data_tmp SELECT fid, dist_fid, ssediam, ssedpercent FROM sed_supply_frac_data"
        )

        self.populate_mud_and_sediment()
        self.functions_cbo.currentIndexChanged.connect(self.function_changed)
    ...
    def sed_transp_restore_defaults(self):
        ...
        self.gutils.clear_tables(
            "sed_group_frac_data_tmp",
            "sed_group_areas_tmp",
            "sed_group_cells_tmp",
            "sed_supply_frac_data_tmp",
        )
    ...
    def is_sediment_transport_ok(self):
        first = "ERROR 281021.0528: Sediment transport data not saved!\n\n"
        wrng = first
        ...
        if self.sed_rating_curve_grp.isChecked() and self.sed_rating_curve_tblw.rowCount() == 0:
            ...
            # Check Chan/FP column in supply rating curve table:
            for row in range(0, self.sed_rating_curve_tblw.rowCount()):
                item_cell = self.sed_rating_curve_tblw.item(row, 1).text()
                if item_cell not in ["0", "1"]:
                    wrng += "* Column Chan/FP (in Sediment Supply Rating Curve table) should be 0 for floodplain or 1 for channel.\n\n"
                    break
                elif item_cell == "":
                    wrng += "* Column Chan/FP (in Sediment Supply Rating Curve table) should be 0 for floodplain or 1 for channel.\n\n"
                    break

            # Check Node column in supply rating curve table:
            for row in range(0, self.sed_rating_curve_tblw.rowCount()):
                item_cell = self.sed_rating_curve_tblw.item(row, 0).text()
                cell = int(item_cell)
                if cell <= 0 or cell > self.n_cells:
                    wrng += "* A cell element is outside the domain (in column 'Node' in Sediment Supply Rating Curve table).\n\n"
                    break
                elif item_cell == "":
                    wrng += "* A cell element is outside the domain (in column 'Node' in Sediment Supply Rating Curve table).\n\n"
                    break
    ...
    def set_sediment_transport(self):
        try:
            ...
            sed_sql = [
                """INSERT INTO sed_groups (isedeqi, bedthick, cvfi, dist_fid) VALUES""",
                4,
            ]
            ...
            self.gutils.execute(
                "INSERT INTO sed_group_frac_data SELECT fid, dist_fid, sediam, sedpercent FROM sed_group_frac_data_tmp"
            )
            ...
            if cells:
                if len(cells) > 0:
                    for i in range(0, len(cells)):
                        ...
                        group_fid = self.gutils.execute(
                            "SELECT group_fid FROM sed_group_areas_tmp WHERE fid = ?;",
                            (area_fid,),
                        ).fetchone()
            ...
            sr_curve_sql = [
                """INSERT INTO sed_supply_areas (isedcfp, ased, bsed, dist_fid) VALUES""",
                4,
            ]
            ...
            for row in range(0, self.sed_rating_curve_tblw.rowCount()):
                ...
                self.gutils.execute(
                    "INSERT INTO sed_supply_cells (grid_fid, area_fid) VALUES (?, ?);",
                    (node, str(row + 1)),
                )
            ...
            self.gutils.execute(
                "INSERT INTO sed_supply_frac_data SELECT fid, dist_fid, ssediam, ssedpercent FROM sed_supply_frac_data_tmp"
            )

            # Rigid Bed Nodes:
            rigid_insert_sql = [
                """INSERT INTO sed_rigid_cells (grid_fid, area_fid) VALUES""",
                2,
            ]
            for row in range(0, self.sed_rigid_nodes_tblw.rowCount()):
    ...
    def set_mud_debris(self):
        ...
        if self.mud_basin_grp.isChecked():
            ...
            self.gutils.execute(
                "INSERT INTO mud_cells (grid_fid, area_fid) VALUES (?, ?);",
                (
                    grid,
                    1,
                ),
    ...
    def sed_size_fraction_dp_tblw_cellchanged(self, row, col):
        ...
        if current_row != -1:
            ...
            if int(current_dist_fid) > 0:
                insert_rt_sql = [
                    """INSERT INTO sed_group_frac_data_tmp (sediam, sedpercent, dist_fid) VALUES""",
                    3,
                ]
                ...
                self.gutils.execute(
                    "DELETE FROM sed_group_frac_data_tmp WHERE dist_fid = ?;",
                    (current_dist_fid,),
                )
                self.gutils.batch_execute(insert_rt_sql)

    # def sed_size_fraction_dp_tblw_changed(self, Qitem):
    #     try:
    ...
    def sed_rating_curve_tblw_cellchanged(self, row, col):
    ...
    def sed_rating_curve_dp_tblw_cellchanged(self, row, col):
        ...
        if current_row != -1:
            ...
            if int(current_dist_fid) > 0:
                insert_rt_sql = [
                    """INSERT INTO sed_supply_frac_data_tmp (ssediam, ssedpercent, dist_fid) VALUES""",
                    3,
                ]
                ...
                self.gutils.execute(
                    "DELETE FROM sed_supply_frac_data_tmp WHERE dist_fid = ?;",
                    (current_dist_fid,),
                )
                self.gutils.batch_execute(insert_rt_sql)
    ...
    def sed_size_grid_tblw_cellchanged(self, row, col):
        ...
        insert_sed_group_areas_sql = [
            """INSERT INTO sed_group_areas_tmp (group_fid) VALUES""",
            1,
        ]
        ...
        sed_group_areas_fid = self.gutils.execute(sed_group_areas_fid_qry, (current_row,)).fetchall()
        if sed_group_areas_fid:
            insert_sed_group_cells_sql = [
                """INSERT INTO sed_group_cells_tmp (grid_fid, area_fid) VALUES""",
                2,
            ]
    ...
    def load_rating_curve_secundary_table(self, dist_fid):
        routing_data_qry = (
            "SELECT ssediam, ssedpercent FROM sed_supply_frac_data_tmp WHERE dist_fid =? ORDER BY ssediam;"
        )
        ...
        if routing_fractions:
            ...
            for row_number, ss_dp in enumerate(routing_fractions):
                ...
                item = QTableWidgetItem(str(ss_dp[0]))
                self.sed_rating_curve_dp_tblw.setItem(row_number, 0, item)
                item = QTableWidgetItem(str(ss_dp[1]))
    ...
    def load_size_fractions_secundary_table(self, dist_fid):
        ...
        if routing_fractions:
            ...
            for row_number, sf_dp in enumerate(routing_fractions):
                ...
                item = QTableWidgetItem(str(sf_dp[0]))
                self.sed_size_fraction_dp_tblw.setItem(row_number, 0, item)
                item = QTableWidgetItem(str(sf_dp[1]))
    ...
    def load_size_fraction_main_table(self):
        ...
        size_fractions = self.gutils.execute(
            "SELECT isedeqi, bedthick, cvfi, dist_fid FROM sed_groups ORDER BY dist_fid"
        ).fetchall()
        if size_fractions:
            self.sed_size_fraction_tblw.setRowCount(0)
    ...
    def load_rating_curve_main_table(self):
        ...
        rating_curve = self.gutils.execute(
            "SELECT fid, isedcfp, ased, bsed, dist_fid FROM sed_supply_areas ORDER BY dist_fid"
        ).fetchall()
        if rating_curve:
            self.sed_rating_curve_tblw.setRowCount(0)
    ...
    def sed_add_size_fraction_cell_btn_clicked(self):
        while True:
            # inGrid = QInputDialog()
            # inGrid.setCancelButtonText("Close")
            # inGrid.setInputMode(QInputDialog.TextInput);
            # inGrid.exec();
    ...
    def sed_add_rating_curve_dp_btn_clicked(self):
        if self.sed_rating_curve_tblw.rowCount() > 0:
            ...
            if self.sed_rating_curve_tblw.item(row, 0):
                if self.sed_rating_curve_tblw.item(row, 0).text().strip() != "":
                    self.sed_rating_curve_dp_tblw.blockSignals(True)
    ...
    def sed_delete_size_fraction_btn_clicked(self):
        if self.sed_size_fraction_tblw.rowCount() > 0:
            ...
            self.sed_size_grid_tblw.blockSignals(True)
            group_areas_fids = self.gutils.execute(
                "SELECT fid FROM sed_group_areas_tmp WHERE group_fid = ?;", (dist_fid,)
            ).fetchall()
            if group_areas_fids:
                for fid in group_areas_fids:
                    self.gutils.execute(
                        "DELETE FROM sed_group_cells_tmp WHERE area_fid  = ?;",
                        (fid[0],),
                    )
            self.gutils.execute("DELETE FROM sed_group_areas_tmp WHERE group_fid = ?;", (dist_fid,))

            self.gutils.execute(
                "UPDATE sed_group_areas_tmp SET group_fid = group_fid - 1 WHERE group_fid > ?;",
                (int(dist_fid),),
            )

            self.gutils.execute("DELETE FROM sed_group_frac_data_tmp WHERE dist_fid = ?;", (dist_fid,))
            self.gutils.execute(
                "UPDATE sed_group_frac_data_tmp SET dist_fid = dist_fid - 1 WHERE dist_fid > ?;",
                (int(dist_fid),),
            )
