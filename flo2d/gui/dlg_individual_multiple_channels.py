...
class IndividualMultipleChannelsDialog(qtBaseClass, uiDialog):
    def __init__(self, iface, lyrs):
        ...
        self.individual_multiple_channel_element_cbo.currentIndexChanged.connect(
            self.individual_mc_element_cbo_currentIndexChanged
        )
    ...
    def save_individual_multiple_chennels_data(self):
        ...
        try:
            ...
            self.uc.show_error(
                "ERROR 210319.0633: update of Individual Multiple Channel Data failed!"
...
class IndividualSimplifiedMultipleChannelsDialog(qtBaseClass, uiDialog):
    ...
    def populate_individual_multiple_cells_dialog(self):
        qry_mc_cells = """SELECT grid_fid FROM simple_mult_cells ORDER BY grid_fid"""
        mc_rows = self.gutils.execute(qry_mc_cells).fetchall()
        if not mc_rows:
            return
