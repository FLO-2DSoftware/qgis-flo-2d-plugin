...
class TributariesDialog(qtBaseClass, uiDialog):
    ...
    def find_chan_confluences(self):
        for row in range(0, self.confluences_tblw.rowCount()):
            ...
            conf_fid = self.gutils.execute(
                "SELECT conf_fid FROM chan_confluences WHERE type = ? AND chan_elem_fid = ?;",
                (
                    0,
                    tributary,
                ),
            ...
            if conf_fid:
                main = self.gutils.execute(
                    "SELECT chan_elem_fid FROM chan_confluences WHERE conf_fid = ? AND type = ?;",
                    (
                        conf_fid[0],
                        1,
                    ),
