...
from qgis.PyQt.QtWidgets import ( QApplication, QComboBox, QCheckBox, QDoubleSpinBox, QInputDialog, QFileDialog, 
                                  qApp, QDialog, QMessageBox,  QSpacerItem, QSizePolicy, QPushButton)
    ...
    def schematize_inlets_and_outfalls(self):
        ...
        try:
            ...
            if self.gutils.is_table_empty('user_swmm_nodes'):
                self.uc.show_warn('User Layer "Storm Drain Nodes" is empty!\n\n' + 
...
#                     msg3 = "No Rating Tables were schematized!\n" 
                ...
                msg = msg1 + msg2 + msg3 
                if msg != "":
                    self.uc.show_info('WARNING 040121.1911: Schematizing Inlets, Outfalls or Rating Tables Storm Drains result:\n\n' + msg)  
            ...
            QApplication.restoreOverrideCursor()
            self.uc.show_error('ERROR 301118..0541: Schematizing Inlets, Outfalls or Rating Tables failed!.'
                               +'\n__________________________________________________', e)               
            return False
    ...
    def schematize_conduits(self):  
        try: 
            ...
            if self.gutils.is_table_empty('user_swmm_conduits'):
                self.uc.show_warn('User Layer "Storm Drain Conduits" is empty!\n\n' + 
                                 'Please import components from .INP file or shapefile, or convert from schematized Storm Drains.')
                return False             
        ...
        try:
            ...
            if storm_drain.create_INP_nodes_dictionary_with_coordinates() == 0:
                ...
                try:
                    ...
                    QApplication.restoreOverrideCursor()
                    self.uc.show_error("ERROR 290220.1727: Reading storm drain time series from SWMM input data failed!"
                               +'\n__________________________________________________', e)   
        ...
        try:
            ...
            for name, values in list(storm_drain.INP_nodes.items()):    # "INP_nodes dictionary contains attributes names and
                ...
                if complete_or_create == "Create New":
                    ...
                    existing_node = self.gutils.execute("SELECT fid FROM user_swmm_nodes WHERE name = ?;", (name,)).fetchone() 
                    if existing_node:      
                        self.gutils.execute(replace_user_swmm_nodes_sql, (junction_invert_elev, max_depth, init_depth, surcharge_depth, ponded_area, outfall_type, 
                                                          outfall_invert_elev, tidal_curve, time_series, flapgate, allow_discharge, 
                                                          junction_invert_elev, max_depth, rim_elev, rim_elev, elev, difference, 
                                                          name)) 
                        updated_nodes += 1 
        ...
        if  complete_or_create == "Create New"  and len(new_nodes) == 0 and len(new_conduits) == 0:
            ...
            if complete_or_create == "Create New":                             
                self.uc.show_info("Importing Storm Drain data finished!\n\n" +
                                  "* " + str(len(new_nodes)) + " nodes (inlets, junctions, and outfalls) were created in the 'Storm Drain Nodes' layer ('User Layers' group), and\n\n" +  
                                  "* " + str(len(new_conduits)) + " conduits in the 'Storm Drain Conduits' layer ('User Layers' group). \n\n"
                ...
                self.uc.show_info("Storm Drain data was updated from file\n" + swmm_file + "\n\n" +
                                  "* " + str(updated_nodes) + " Nodes (inlets, junctions, and outfalls) in the 'Storm Drain Nodes' layer ('User Layers' group) were updated, and\n\n" +  
                                  "* " + str(updated_conduits) + " Conduits in the 'Storm Drain Conduits' layer ('User Layers' group) were updated. \n\n"
                                  "Click the 'Inlets/Junctions', 'Outfalls', and 'Conduits' buttons in the Storm Drain Editor widget to see or edit their attributes.\n\n"
                                  "NOTE: the 'Schematize Storm Drain Components' button  in the Storm Drain Editor widget will update the 'Storm Drain' layer group, required to "
                                             "later export the .DAT files used by the FLO-2D model.")                
            if outside_nodes != "":
                    ...
                    try:
                        ...
                        if not conduits_rows:
                            ...
                            for row in conduits_rows:
                                row = (row[0], "?" if row[1] is None else row[1], "?" if row[2] is None else row[2], 0 if row[3] is None else row[3],
                                        0 if row[4] is None else row[4], 0 if row[5] is None else row[5],  0 if row[6] is None else row[6],
                                         0 if row[7] is None else row[7],  0 if row[8] is None else row[8])  
                                if row[1] == "?" or row[2] == "?":
...
#                         swmm_inp_file.write('\n')                     
    ...
    def show_inlets(self):
        ...
        rows = self.gutils.execute(qry).fetchall()
        if not rows:
            self.uc.show_info("WARNING 280920.0422: No inlets/junctions defined (of type 'I' or 'J') in 'Storm Drain Nodes' User Layer!")            
            return
        ...
        try:
            ...
            for file in rating_files:
                ...
                if err0 == ""  and err1 == ""  and err2 == "":
                    ...
                    self.add_rtable(file_name)  # Rating table 'file_name' is deleted from 'swmmflort' and its data from 'swmmflort_data' if they exist. 
...
#                     raise Exception("NOTE3: update rt_fid")   
                    ...
                    accepted_files.append(file_name + file_ext + " rating table was assigned to inlet with identical name")                                             
                else:
                    if err0:
                        errors0.append(err0) 
                    if err1:
            ...
            if lst_no_type4:
                QApplication.restoreOverrideCursor()
                answer = self.uc.question(str(goodRT) + " imported rating tables were assigned to inlets with identical name.\n\n" +
                                    "Of those " + str(goodRT) + " inlets, " + str(len(lst_no_type4)) + 
...
#                     lst_no_type4  = [] 
#                     str_no_type4 = "" 
                    txt2 = "* " + str(len(lst_no_type4)) + " inlet's drain type changed to type 4 (stage-discharge).\n\n"
                else:
                    if len(lst_no_type4) > 1:
                        txt2 = "* " + str(len(lst_no_type4)) + " inlet's drain type are not of type 4 (stage-discharge) but have rating table assigned.\n\n" 
                    else:
                        txt2 = "* " + str(len(lst_no_type4)) + " inlet's drain type is not of type 4 (stage-discharge) but has rating table assigned.\n\n"                  
                    self.uc.show_warn("WARNING 121220.1856:\n\n" + 
...
#                 noInlets.append("\n")   
            ...
            if len_errors  + len(noInlets)  + goodRT > 0:
                with open(last_dir + "\Rating Tables Warnings.CHK", 'w') as error_file:
                    ...
                    if str_no_type4 != "":
                        if answer:   
                            error_file.write( "\nThe following inlets were assigned rating tables and its Drain type changed to 4 (stage-discharge):" + str_no_type4 + "\n") 
                        else:
                            error_file.write( "\nThe following inlets were assigned rating tables but are not of type 4 (stage-discharge):" + str_no_type4 + "\n") 
                ...
                self.uc.show_info("INFO 181120.1629:\n\n" + "* " + str(len(rating_files)) + " files selected" + 
                                  (", of which " + str(len_errors) + txt1 if len_errors > 0 else ".\n\n") +
                                  ("* " + str(len(noInlets)) + " rating tables were not read (no inlets with identical name).\n\n" if len(noInlets) > 0 else "") +
                                  "* " + str(goodRT) + " rating tables were assigned to inlets with identical name.\n\n" +  txt2 + 
                                  "See details in file\n\n" +  os.path.dirname(rating_files[0]) + "/Rating Tables Warnings.CHK")
        ...
        if save:
            try:
                if dlg_shapefile.saveSelected:
                    self.uc.bar_info("Storm drain components (inlets, outfalls, and/or conduits) from hydraulic layers saved.")
