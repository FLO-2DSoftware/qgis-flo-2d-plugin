...
GDALGRID_METHOD_DESC = ('Average','Inverse Distance','Inverse Distance Nearest Neighbor','Nearest Neighbor','Linear')
GDALGRID_METHOD_PARAMS = ('power','smoothing','angle','radius','radius1','radius2','min points', 'max points', 'nodata')
GDALGRID_METHOD_PARAM_DICT = { 'Average': ('radius1','radius2','angle','min points','nodata'),
...
class ResamplingOption:
    ...
    def string(self):  
        if self.method == 'invdist':
            result = '%s:power=%s:smoothing=%s:radius1=%s:radius2=%s:'\
                     'angle=%s:min_points=%s:max_points=%s:nodata=%s'\
                     ''%(self.method,self.power,self.smoothing,self.radius1,self.radius2,
                         self.angle,self.min_points,self.max_points,self.nodata)

        elif self.method == 'invdistnn':
            result = '%s:power=%s:smoothing=%s:radius=%s:'\
                     'min_points=%s:max_points=%s:nodata=%s'\
                     ''%(self.method,self.power,self.smoothing,self.radius,
                         self.min_points,self.max_points,self.nodata)

        elif self.method == 'average':
            result = '%s:radius1=%s:radius2=%s:'\
                     'angle=%s:min_points=%s:nodata=%s'\
                     ''%(self.method,self.radius1,self.radius2,
                         self.angle,self.min_points,self.nodata)

        elif self.method == 'nearest':
            result = '%s:radius1=%s:radius2=%s:'\
                     'angle=%s:nodata=%s'\
                     ''%(self.method,self.radius1,self.radius2,
                         self.angle,self.nodata)

        elif self.method == 'linear':
            result = '%s:radius=%s:'\
                     'nodata=%s'\
                     ''%(self.method,self.radius,
                         self.nodata)
        else:
...
def clip_raster_aligned(raster_inpath,raster_outpath,clip_extents):
    ...
    opt = gdal.TranslateOptions(srcWin=[ul_col,ul_row,dst_cols,dst_rows], 
                                format='GTiff',
...
def raster_to_xyz(raster_inpath, xyz_outpath, set_integer_nodata = False, remove_nodata = True):
    ...
    if set_integer_nodata:
        ...
        warp_opt = gdal.WarpOptions(dstNodata = src_nodata,
    ...
    def _raster_to_xyz():
        ...
        trans_opt = gdal.TranslateOptions(creationOptions = ['DECIMAL_PRECISION=4',
    ...
    def _remove_xyz_nodata():
        ...
        if dask is None:
            ...
            ogr_opt = gdal.VectorTranslateOptions(format = 'CSV',
                                                  accessMode = 'overwrite',
                                                  SQLStatement = "SELECT CAST(X as float),CAST(Y as float),CAST(Z as float) FROM {0:s} WHERE Z > '{1:f}'".format(os.path.basename(xyz_outpath)[:-4], src_nodata),
            ...
            df = dask.read_csv(xyz_outpath)
            df_revised = df[df.Z > src_nodata]
            df_revised.to_csv(xyz_temp_outpath,
                              single_file = True,
                              index = False)
    if remove_nodata:        
        _remove_xyz_nodata()        
...
if __name__ == '__main__':
    ...
    srs = r"+proj=utm +zone=48 +ellps=WGS84 +datum=WGS84 +units=m +no_defs" 
    rows = int((ymax - ymin)/cell_size) 
    cols = int((xmax - xmin)/cell_size)
    shape = (rows,cols)
    resample = ResamplingOption('Average',radius1=10,radius2=10,angle=0,min_points=1,max_points=0,nodata=-9999) 
    raster_src_file = r"C:\projects\FLO-2D\Lesson1 welev\Elevation.tif" 
    raster_clip_file = r"C:\projects\FLO-2D\Lesson1 welev\Elevation_clipped.tif" 
    xyz_file = os.path.join(r"C:\projects\FLO-2D\Lesson1 welev","Elevation_xyz.csv")
    xyz_nodata_file = os.path.join(r"C:\projects\FLO-2D\Lesson1 welev","Elevation_xyz_nodata.csv")
    raster_grid = r"C:\projects\FLO-2D\Lesson1 welev\Elevation_xyz_gdalgrid.tif" 
    clip_raster_aligned(raster_src_file, raster_clip_file, extents)
    raster_to_xyz(raster_clip_file, xyz_file, False)
    raster_outpath = xyz_to_raster(xyz_file,
                                   extents = extents,
                                   shape = shape,
