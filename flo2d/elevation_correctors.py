...
class LeveesElevation(ElevationCorrector):
    ...
    def setup_layers(self):
        self.points = self.lyrs.data['user_levee_points']['qlyr']
        self.lines = self.lyrs.data['user_levee_lines']['qlyr']
        self.polygons = self.lyrs.data['user_levee_polygons']['qlyr']
...
class GridElevation(ElevationCorrector):
    ...
    def elevation_from_tin(self):
        ...
        cur = self.gutils.con.cursor()
        for feat in self.schematic.getFeatures(grid_fids):
            geom = feat.geometry()
            centroid = geom.centroid().asPoint()
            value = self.tin.tin_at_xy(centroid.x(), centroid.y())
